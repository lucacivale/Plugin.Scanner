    ...
    where TOptions : IScanOptions
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="Scanner{TOptions}"/> class.
    /// </summary>
    /// <param name="currentActivity">The current activity provider.</param>
    protected Scanner(ICurrentActivity currentActivity)
    {
    ...
    public async Task<IScanResult> ScanAsync(TOptions options, CancellationToken cancellationToken)
    {
        ...
        handler.Post(async () =>
        {
            try
            {
                ...
                using IDataDetector dataDetector = CreateDataDetector(options);
                using MlKitAnalyzer analyzer = new([dataDetector.Detector], ImageAnalysis.CoordinateSystemViewReferenced, mainExecutor, dataDetector);
                ...
                using DataScannerDialog scannerDialog = CreateDataScannerDialog(
    ...
    protected abstract DataScannerDialog CreateDataScannerDialog(
        Activity activity,
